...
const DetailPage = () => {
  ...
  const filterAndFormatDescribtion = (flavorText: FlavorTextEntry[]): string[] => {
  ...
  const getPokemonDescribtion = async (id: number): Promise<string> => {
    const url = `https://pokeapi.co/api/v2/pokemon-species/${id}/`

    const {data: pokemonSpecies} = await axios.get<PokemonDescription>(url);
    // console.log('@@@@@@@@@',JSON.stringify(pokemonSpecies));

    const describtions: string[] = filterAndFormatDescribtion(pokemonSpecies.flavor_text_entries);
